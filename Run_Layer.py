...
def main(args):
    ...
    if args.run_trt:
        ...
        Layer = nvbitfi_DNN.TRT_load_embeddings(path_dir=f"{args.type}/{args.model_name}",batch_size=batch_size, layer_output_shape=shape)
        Layer.TRT_layer_inference()
    else:
        import nvbitfi_DNN as nvbitfi_DNN
        Layer = nvbitfi_DNN.load_embeddings(path_dir=f"{args.type}/{args.model_name}",batch_size=batch_size)
